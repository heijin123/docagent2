...
flowchart LR
    ...
    F --> G[质量校验]
    G -->|通过| H[输出给 Chunking]
    G -->|不通过| I[标记 parse_warning<br/>转人工复核]
```
...
| F3.6 | **死循环防护**：retry 计数写入 state，max_retry=2，超限强制走 Handoff 降级路径 | P0 |
| F3.7 | **Handoff 节点**：输出兜底话术 + 建议转人工，标记 `degraded=true` | P0 |
| F3.8 | 确定性逻辑（重试次数、置信度阈值判断）写在图的条件边里，**不让 LLM 决定流程走向** | P0 |
