...
# ── F3.1 supervisor ─────────────────────────────────────────
def supervisor_prompt(query: str, history: str) -> tuple[str, str]:
    system = (
        "你是企业知识问答系统的意图分类器。仅做分类，不做回答。\n"
        "输出 JSON，intent 只允许取值："
        + _dumps(["kb_qa", "chitchat", "human_handoff"])
        + "，含义：\n"
        "- kb_qa：问题依赖企业知识库（制度/流程/政策/数据报表等），需要检索回答；\n"
        "- chitchat：寒暄/无知识依赖的闲聊（你好、谢谢、再见等）；\n"
        "- human_handoff：用户明确要求转人工/找客服/投诉，或涉及敏感需人工的场景。\n"
        "输出示例：" + _dumps({"intent": "kb_qa", "reason": "询问报销流程，属于制度类问题"})
    )
    user = "历史对话（供上下文，可能为空）：\n" + history + "\n\n<query>" + query + "</query>"
    return system, user
...
# ── F3.4 answer（含 F3.9 过期约束 / 资料不足直说）──────────────
def answer_prompt(query: str, evidence: str, history: str,
    ...
    system = ("你是企业知识库问答助手。输出 JSON：answer（最终回答文本）与 "
              "chunk_ids（本次回答引用的证据 chunk_id 列表）。\n"
              + "\n".join(rule) + "\n输出示例：" +
              _dumps({"answer": "根据《考勤管理制度》……（含 [来源: ...] 标记）",
...
def answer_stream_prompt(query: str, evidence: str, history: str,
    ...
    """M4 SSE 流式 answer：纯文本输出（非 JSON），引用标记内嵌文本。
    ...
    rule = [
        "回答约束：",
        "1. 直接输出最终回答文本，不要输出 JSON 或任何包裹格式；",
    ...
    if include_expired:
        rule.append("6. 用户已确认可查看过期文档，但过期引用仍须遵守第 4/5 条失效提示。")
    system = "你是企业知识库问答助手。\n" + "\n".join(rule)
    user = (
        "历史对话（语境参考）：\n" + history + "\n\n"
        "<query>" + query + "</query>\n\n"
